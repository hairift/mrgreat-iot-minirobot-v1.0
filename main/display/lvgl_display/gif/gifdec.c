...
static gd_GIF * gif_open(gd_GIF * gif_base)
{
    ...
    /* Kedalaman ruang warna */
    depth = ((fdsz >> 4) & 7) + 1;
    /* Abaikan penanda pengurutan. */
    /* Ukuran GCT */
    gct_sz = 1 << ((fdsz & 0x07) + 1);
    /* Indeks warna latar belakang */
    f_gif_read(gif_base, &bgidx, 1);
    /* Rasio aspek */
    f_gif_read(gif_base, &aspect, 1);
    /* Buat struktur gd_GIF. */
    if(0 == width || 0 == height){
    ...
    for(int i = 0; i < gif->width * gif->height; i++) {
        ...
        gif->canvas[i * 4 + 3] = 0x00;  // Mulai dari transparan agar frame pertama dirender sesuai transparansinya sendiri
    }
#endif
...
read_plain_text_ext(gd_GIF * gif)
{
    if(gif->plain_text) {
        ...
        uint8_t cw, ch, fg, bg;
        size_t sub_block;
        f_gif_seek(gif, 1, LV_FS_SEEK_CUR); /* ukuran blok = 12 */
        tx = read_num(gif);
        ty = read_num(gif);
        tw = read_num(gif);
        th = read_num(gif);
        ...
        /* Abaikan metadata teks polos. */
        f_gif_seek(gif, 13, LV_FS_SEEK_CUR);
    }
    /* Abaikan subblok teks polos. */
...
read_graphic_control_ext(gd_GIF * gif)
{
    ...
    /* Abaikan ukuran blok (selalu 0x04). */
...
read_application_ext(gd_GIF * gif)
{
    ...
    /* Abaikan ukuran blok (selalu 0x0B). */
    f_gif_seek(gif, 1, LV_FS_SEEK_CUR);
    /* Pengenal aplikasi. */
    ...
    if(!strncmp(app_id, "NETSCAPE", sizeof(app_id))) {
        /* Abaikan ukuran blok (0x03) dan byte konstan (0x01). */
...
get_key(gd_GIF *gif, int key_size, uint8_t *sub_len, uint8_t *shift, uint8_t *byte)
{
    ...
    for (bits_read = 0; bits_read < key_size; bits_read += frag_size) {
        ...
        if (rpad == 0) {
            /* Perbarui byte. */
            if (*sub_len == 0) {
                f_gif_read(gif, sub_len, 1); /* Harus bernilai bukan nol. */
    ...
    /* Bersihkan bit tambahan di sisi kiri. */
    key &= (1 << key_size) - 1;
...
#if LV_GIF_CACHE_DECODE_DATA
/* Uraikan kompresi piksel gambar.
 * Mengembalikan 0 jika berhasil atau -1 jika kehabisan memori
 * (terkait tabel kode LZW) atau terjadi kesalahan parsing. */
static int
read_image_data(gd_GIF *gif, int interlace)
{
    ...
    /* Nilai pertama dari urutan nilai yang terkait dengan key */
    int first_value;
    int last_key;
    uint8_t *sp = NULL;
    ...
    /* Ambil ukuran kunci awal, kode clear, dan kode stop */
    f_gif_read(gif, &byte, 1);
    key_size = (int) byte;
    ...
    while (frm_off < frm_size) {
        ...
        curr_code = key;
        /*
         * Jika kode saat ini adalah kode yang akan ditambahkan ke kamus
         * dekode, isinya dibentuk dari daftar data milik kunci sebelumnya
         * dan data pertamanya.
         * */
        ...
        *sp++ = curr_code;
        ...
        /* Tambahkan kode ke kamus dekode */
    ...
    if (key == stop_code) f_gif_read(gif, &sub_len, 1); /* Harus bernilai nol. */
...
/* Tambahkan entri tabel. Nilai kembali:
 *  0 jika berhasil
 *  +1 jika ukuran kunci harus dinaikkan setelah penambahan ini
 *  -1 jika tabel tidak bisa dialokasikan ulang */
static int
...
/* Hitung indeks keluaran untuk baris masukan ke-y pada bingkai setinggi h. */
static int
interlaced_line_index(int h, int y)
{
    int p; /* jumlah baris pada lintasan saat ini */
...
read_image_data(gd_GIF * gif, int interlace)
{
    ...
    sub_len = shift = 0;
    key = get_key(gif, key_size, &sub_len, &shift, &byte); /* kode clear */
    frm_off = 0;
    ret = 0;
    frm_size = gif->fw * gif->fh;
    while(frm_off < frm_size) {
...
dispose(gd_GIF * gif)
{
    ...
    switch(gif->gce.disposal) {
        case 2: /* Kembalikan ke warna latar belakang. */
            bgcolor = &gif->palette->colors[gif->bgindex * 3];
...
#endif
            break;
        case 3: /* Kembalikan ke kondisi sebelumnya, artinya kanvas tidak diperbarui. */
            break;
        default:
            /* Tambahkan piksel bingkai yang tidak transparan ke kanvas. */
            render_frame_rect(gif, gif->canvas);
    }
}

/* Mengembalikan 1 jika mendapat bingkai; 0 jika mencapai trailer GIF; -1 jika terjadi kesalahan. */
int
