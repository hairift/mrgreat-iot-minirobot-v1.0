...
protected:
    lv_obj_t* high_temp_popup_ = nullptr;  // Jendela peringatan suhu tinggi
    lv_obj_t* high_temp_label_ = nullptr;  // Label peringatan suhu tinggi

public:
    // Gunakan konstruktor dari kelas induk
    using SpiLcdDisplay::SpiLcdDisplay;

    void SetupHighTempWarningPopup() {
        auto lvgl_theme = static_cast<LvglTheme*>(current_theme_);
        auto text_font = lvgl_theme->text_font()->font();
        // Buat jendela peringatan suhu tinggi
        high_temp_popup_ = lv_obj_create(lv_screen_active());  // Gunakan layar yang sedang aktif
