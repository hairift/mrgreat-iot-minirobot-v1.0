...
typedef struct {
    ...
    const gc9503_lcd_init_cmd_t *init_cmds;         /*!< Penunjuk ke larik perintah inisialisasi. Atur ke NULL bila memakai perintah bawaan.
    ...
    struct {
        unsigned int mirror_by_cmd: 1;              /*<! Fungsi `mirror()` akan dijalankan lewat perintah LCD jika diatur ke 1.
                                                     *   Jika tidak, fungsi ini akan dijalankan oleh perangkat lunak.
                                                     */
        unsigned int auto_del_panel_io: 1;          /*<! Hapus instance panel IO secara otomatis jika diatur ke 1. Semua flag `*_by_cmd` menjadi tidak berlaku.
                                                     *   Jika pin panel IO berbagi dengan pin lain pada antarmuka RGB untuk menghemat GPIO,
                                                     *   atur ke 1 agar panel IO beserta pin-pinnya dilepas (kecuali sinyal CS).
                                                     */
    } flags;
 ...
 *
 * @note  Saat `auto_del_panel_io` diatur ke 1, fungsi ini akan menginisialisasi GC9503 lebih dulu dengan inisialisasi khusus vendor lalu memanggil `esp_lcd_new_rgb_panel()` untuk membuat panel LCD RGB. Fungsi `esp_lcd_panel_init()` setelah itu hanya menginisialisasi bagian RGB.
 * @note  Saat `auto_del_panel_io` diatur ke 0, fungsi ini hanya memanggil `esp_lcd_new_rgb_panel()` untuk membuat panel LCD RGB. Fungsi `esp_lcd_panel_init()` setelah itu akan menginisialisasi GC9503 dan RGB sekaligus.
 * @note  Inisialisasi khusus vendor dapat berbeda antar pabrikan, jadi sebaiknya rujuk ke pemasok LCD untuk kode urutan inisialisasinya.
 *
...
/**
 * @brief Struktur konfigurasi panel IO SPI 3-kawat
 *
