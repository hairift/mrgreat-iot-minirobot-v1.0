...
Backlight::Backlight() {
    // Buat pewaktu transisi halus untuk lampu latar.
...
void Backlight::RestoreBrightness() {
    // Muat tingkat kecerahan dari pengaturan.
    Settings settings("display");  
    int saved_brightness = settings.GetInt("brightness", 75);
    
    // Jika nilai kecerahan nol atau terlalu kecil, pakai nilai aman bawaan.
    if (saved_brightness <= 0) {
        ESP_LOGW(TAG, "Brightness value (%d) is too small, setting to default (10)", saved_brightness);
        saved_brightness = 10;  // Gunakan nilai bawaan yang tetap nyaman untuk layar.
    }
...
void Backlight::SetBrightness(uint8_t brightness, bool permanent) {
    ...
    if (transition_timer_ != nullptr) {
        // Jalankan pewaktu dan perbarui kecerahan tiap 5 ms.
...
PwmBacklight::PwmBacklight(gpio_num_t pin, bool output_invert, uint32_t freq_hz) : Backlight() {
    const ledc_timer_config_t backlight_timer = {
        ...
        .freq_hz = freq_hz, // Frekuensi PWM dibuat cukup tinggi agar induktor tidak mendesis.
        .clk_cfg = LEDC_AUTO_CLK,
        .deconfigure = false
    };
    ESP_ERROR_CHECK(ledc_timer_config(&backlight_timer));

    // Siapkan periferal LEDC untuk mengendalikan lampu latar berbasis PWM.
...
void PwmBacklight::SetBrightnessImpl(uint8_t brightness) {
    // Resolusi LEDC adalah 10 bit, sehingga 100 persen setara dengan 1023.
