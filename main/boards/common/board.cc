...
std::string Board::GenerateUuid() {
    // UUID v4 membutuhkan 16 byte data acak.
    uint8_t uuid[16];

    // Gunakan pembangkit angka acak perangkat keras milik ESP32.
    esp_fill_random(uuid, sizeof(uuid));

    // Atur bit versi dan varian UUID.
    uuid[6] = (uuid[6] & 0x0F) | 0x40;    // Versi 4.
    uuid[8] = (uuid[8] & 0x3F) | 0x80;    // Varian 1.

    // Ubah susunan byte menjadi format string UUID standar.
    char uuid_str[37];
...
std::string Board::GetSystemInfoJson() {
    ...
    */
    ...
    json.pop_back(); // Hapus koma terakhir.
    json += R"(],)";
