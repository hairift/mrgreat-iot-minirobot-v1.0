...
public:
    ...
    void Printcharge()
    {
        ...
        // Gunakan variabel ini agar tidak memunculkan peringatan kompilasi.
        (void)voltage;
        (void)current;
    }
...
public:
    ...
    Cst816s(i2c_master_bus_handle_t i2c_bus, uint8_t addr) : I2cDevice(i2c_bus, addr)
    {
        ...
        // Buat semaphore untuk interupsi sentuh
    ...
    ~Cst816s()
    {
        ...
        // Hapus semaphore jika sudah ada
        if (touch_isr_mux_ != NULL) {
    ...
    TouchEvent CheckTouchEvent()
    {
        ...
        if (is_touched && !was_touched_) {
            ...
            // Sentuhan berkelanjutan saat jari masih menempel
            event = TOUCH_HOLD;
            ESP_LOGD(TAG, "TOUCH HOLD - x: %d, y: %d", tp_.x, tp_.y);
        }

        // Perbarui status sentuhan sebelumnya
...
private:
    ...
    esp_lcd_touch_handle_t tp;   // Objek sentuh LCD
    ...
    void InitializeButtons()
    {
        boot_button_.OnClick([this]() {
            auto &app = Application::GetInstance();
            if (app.GetDeviceState() == kDeviceStateStarting) {
                ESP_LOGI(TAG, "Tombol BOOT ditekan, masuk ke mode konfigurasi Wi-Fi");
                EnterWifiConfigMode();
                return;
            }
            app.ToggleChatState();
...
public:
    ~EspVocat() {
        // Hentikan tugas yang masih berjalan
        ...
        // Hapus objek yang dimiliki papan
        delete charge_;
        delete cst816s_;
        delete display_;
        // Catatan: backlight_ (PwmBacklight) dan camera_ (EspVideo) tidak dihapus di sini
        // karena kelas dasar (Backlight, Camera) tidak memiliki destruktor virtual.
        // Karena EspVocat adalah singleton yang hidup sepanjang umur perangkat, ini masih dapat diterima.

        // Lepas handler ISR GPIO
        gpio_isr_handler_remove(TP_PIN_NUM_INT);

        // Nonaktifkan sensor suhu
        if (temp_sensor != NULL) {
