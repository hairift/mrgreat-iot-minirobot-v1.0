...
private:
    ...
    std::optional<int> min_value_;  // Tambahan: nilai minimum untuk integer
    std::optional<int> max_value_;  // Tambahan: nilai maksimum untuk integer

public:
    // Konstruktor untuk field wajib
    Property(const std::string& name, PropertyType type)
        : name_(name), type_(type), has_default_value_(false) {}

    // Konstruktor untuk field opsional dengan nilai bawaan
    template<typename T>
    ...
    inline void set_value(const T& value) {
        // Tambahkan pemeriksaan rentang saat mengatur nilai integer
...
public:
    ...
    void set_run_on_main_thread(bool run_on_main_thread) { run_on_main_thread_ = run_on_main_thread; }
    inline const std::string& name() const { return name_; }
    inline const std::string& description() const { return description_; }
    inline const PropertyList& properties() const { return properties_; }
    inline bool user_only() const { return user_only_; }
    inline bool run_on_main_thread() const { return run_on_main_thread_; }
    ...
    std::string to_json() const {
        ...
        // Tambahkan anotasi audience jika tool hanya untuk pengguna dan tidak terlihat oleh AI
        if (user_only_) {
...
private:
    ...
    void ExecuteToolCall(int id, McpTool* tool, const PropertyList& arguments);
