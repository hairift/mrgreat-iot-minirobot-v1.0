...
esp_err_t SystemInfo::PrintTaskCpuUsage(TickType_t xTicksToWait) {
    ...
    // Alokasikan larik untuk menyimpan status tugas saat ini
    ...
    // Alokasikan larik untuk menyimpan status tugas setelah jeda
    ...
    // Ambil status tugas setelah jeda
    ...
    // Hitung total waktu berlalu dalam satuan periode clock statistik waktu jalan
    ...
    // Cocokkan setiap tugas di start_array dengan data pasangannya di end_array
    for (int i = 0; i < start_array_size; i++) {
        int k = -1;
        for (int j = 0; j < end_array_size; j++) {
            if (start_array[i].xHandle == end_array[j].xHandle) {
                k = j;
                // Tandai tugas yang sudah cocok dengan menimpa handle miliknya
                start_array[i].xHandle = NULL;
                end_array[j].xHandle = NULL;
                break;
            }
        }
        // Periksa apakah tugas yang cocok ditemukan
        if (k >= 0) {
    ...
    // Cetak tugas yang tidak mempunyai pasangan
    for (int i = 0; i < start_array_size; i++) {
