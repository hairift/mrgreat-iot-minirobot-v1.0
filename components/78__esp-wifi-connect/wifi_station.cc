...
void WifiStation::Stop() {
    ...
    // Lepas penangan peristiwa lebih dulu agar hasil pemindaian tidak memicu koneksi baru.
    ...
    // Setelah penangan dilepas, pemindaian dan Wi-Fi aman untuk dihentikan.
    esp_wifi_scan_stop();
    esp_wifi_disconnect();
    esp_wifi_stop();
    ...
    // Reset status koneksi lama agar sesi berikutnya tidak ikut terpengaruh.
    was_connected_ = false;

    // Hapus bit connected.
    xEventGroupClearBits(event_group_, WIFI_EVENT_CONNECTED);

    // Set bit stopped setelah pembersihan selesai agar WaitForConnected tidak menggantung.
    // Ini mencegah race condition dengan operasi Wi-Fi berikutnya.
...
void WifiStation::Start() {
    // Catatan: esp_netif_init() dan esp_wifi_init() cukup dipanggil sekali sebelum fungsi ini.
    // Driver Wi-Fi diinisialisasi oleh WifiManager::Initialize() dan tetap aktif.

    // Bersihkan bit stopped agar WaitForConnected bekerja dengan benar.
    // Bersihkan bit penanda pemindaian selesai agar Stop() bisa menunggu pemindaian berakhir.
    xEventGroupClearBits(event_group_, WIFI_EVENT_STOPPED | WIFI_EVENT_SCAN_DONE_BIT);

    // Setel ulang interval pemindaian ke nilai minimum agar koneksi awal lebih cepat.
    scan_current_interval_microseconds_ = scan_min_interval_microseconds_;

    // Buat antarmuka stasiun Wi-Fi bawaan.
    ...
    // Siapkan pewaktu untuk melakukan pemindaian Wi-Fi.
...
bool WifiStation::WaitForConnected(int timeout_ms) {
    // Tunggu sampai perangkat terhubung atau proses dihentikan.
    ...
    // Kembalikan true hanya jika benar-benar terhubung.
...
void WifiStation::HandleScanResult() {
    ...
    // Urutkan berdasarkan RSSI dari yang paling kuat.
    ...
    ESP_LOGI(TAG, "Scan found %d APs, saved SSID count: %d", ap_num, (int)ssid_list.size());
...
int8_t WifiStation::GetRssi() {
    // Pastikan perangkat sedang terhubung terlebih dahulu.
    if (!IsConnected()) {
        return 0;  // Kembalikan 0 jika belum terhubung.
    }

    // Ambil informasi titik akses yang sedang digunakan.
...
void WifiStation::UpdateScanInterval() {
    // Terapkan exponential backoff dengan menggandakan interval sampai batas maksimum.
...
// Fungsi penangan peristiwa statis.
void WifiStation::WifiEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data) {
    ...
    if (event_id == WIFI_EVENT_STA_START) {
        ...
        // Panggil fungsi panggil balik putus koneksi sekali saja saat status berubah dari tersambung ke terputus.
...
void WifiStation::IpEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data) {
    ...
    this_->was_connected_ = true;  // Tandai sudah pernah terhubung untuk notifikasi saat terputus.
    ...
    // Setel ulang interval pemindaian ke nilai minimum agar penyambungan ulang berikutnya lebih cepat.
