...
void WifiConfigurationAp::Start()
{
    ...
    // Mulai pemindaian segera
    esp_wifi_scan_start(nullptr, false);
    // Siapkan pewaktu pemindaian Wi-Fi berkala
...
{
    // Alamat portal konfigurasi
    return "http://192.168.4.1";
}

void WifiConfigurationAp::StartAccessPoint()
{
    // Catatan: esp_netif_init() dan esp_wifi_init() harus dipanggil sekali sebelum metode ini
    // Penggerak Wi-Fi diinisialisasi oleh WifiManager::Initialize() dan tetap aktif
    
    // Buat antarmuka AP Wi-Fi bawaan
    ap_netif_ = esp_netif_create_default_wifi_ap();

    // Atur alamat IP AP menjadi 192.168.4.1
...
{
    // Jalankan peladen web
    httpd_config_t config = HTTPD_DEFAULT_CONFIG();
    config.max_uri_handlers = 24;
    config.uri_match_fn = httpd_uri_match_wildcard;
    // Jaringan 5G membutuhkan waktu sambung lebih lama
    config.recv_wait_timeout = 15;
    config.send_wait_timeout = 15;
    ESP_ERROR_CHECK(httpd_start(&server_, &config));

    // Daftarkan halaman index.html
    ...
    httpd_uri_t saved_list = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            if (json_str.length() > 1) {
                json_str.pop_back(); // Hapus koma terakhir
            }
            json_str += "]";
    ...
    // Daftarkan URI /saved/set_default
    ...
    httpd_uri_t scan = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            // Periksa apakah 5G didukung
            bool support_5g = false;
#ifdef CONFIG_SOC_WIFI_SUPPORT_5G
            support_5g = true;
#endif

            // Kirim hasil pemindaian sebagai JSON
            httpd_resp_set_type(req, "application/json");
            httpd_resp_set_hdr(req, "Connection", "close");
    ...
    // Daftarkan penangan pengiriman formulir
    httpd_uri_t form_submit = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            if (buf_len > 1024) { // Batasi ukuran badan permintaan maksimum
            ...
            // Keluar otomatis dari mode konfigurasi setelah penyimpanan berhasil
            // Ini memastikan perangkat tetap keluar meski pengguna tidak menekan tombol keluar
            ESP_LOGI(TAG, "Auto-exit config mode after successful WiFi save");
            xTaskCreate([](void *ctx) {
                // Tunggu 3 detik agar respons HTTP terkirim
                // dan pengguna sempat melihat pesan berhasil
                vTaskDelay(pdMS_TO_TICKS(3000));
                
                auto* self = static_cast<WifiConfigurationAp*>(ctx);
                if (self->on_exit_requested_) {
                    ESP_LOGI(TAG, "Triggering auto-exit from config mode");
                    self->on_exit_requested_();
                }
                vTaskDelete(NULL);
            }, "auto_exit_cfg", 4096, this_, 5, NULL);
    ...
    // Daftarkan rute keluar tanpa memulai ulang perangkat
    httpd_uri_t exit_config = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            // Atur header respons agar peramban tidak menyimpan tembolok
            httpd_resp_set_type(req, "application/json");
            ...
            // Tunda pemanggilan fungsi panggil balik agar respons HTTP terkirim penuh
            ESP_LOGI(TAG, "Exiting config mode...");
            xTaskCreate([](void *ctx) {
                // Tunggu 200 ms agar respons HTTP benar-benar terkirim
                vTaskDelay(pdMS_TO_TICKS(200));
                
                auto* self = static_cast<WifiConfigurationAp*>(ctx);
                // Beri tahu fungsi panggil balik untuk keluar dari mode konfigurasi
                if (self->on_exit_requested_) {
                    self->on_exit_requested_();
                }
                vTaskDelete(NULL);
    ...
    auto captive_portal_handler = [](httpd_req_t *req) -> esp_err_t {
        ...
        // Atur tipe konten agar tidak memunculkan peringatan di peramban
    ...
    // Daftarkan semua rute umum untuk deteksi portal tangkapan
    const char* captive_portal_urls[] = {
        "/hotspot-detect.html",    // Perangkat Apple
        "/generate_204*",          // Perangkat Android
        "/mobile/status.php",      // Perangkat Android
        "/check_network_status.txt", // Windows
        "/ncsi.txt",               // Windows
        "/fwlink/",                // Microsoft
        "/connectivity-check.html", // Firefox
        "/success.txt",            // Berbagai perangkat
        "/portal.html",            // Berbagai perangkat
        "/library/test/success.html" // Perangkat Apple
    };
    ...
    // Daftarkan URI untuk pengaturan lanjutan
    ...
    httpd_uri_t advanced_submit = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            // Simpan pengaturan pengingat BSSID
...
bool WifiConfigurationAp::ConnectToWifi(const std::string &ssid, const std::string &password)
{
    ...
    if (ssid.length() > 32) {  // Panjang maksimum SSID Wi-Fi
    ...
    // Tunggu sambungan selesai selama 10 atau 25 detik
...
void WifiConfigurationAp::WifiEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    ...
    if (event_id == WIFI_EVENT_AP_STACONNECTED) {
        ...
        // Setelah pemindaian selesai, tunggu 10 detik sebelum memindai lagi
...
{
    // Daftarkan penangan peristiwa konfigurasi pintar
    ...
    // Inisialisasi konfigurasi konfigurasi pintar
    smartconfig_start_config_t cfg = SMARTCONFIG_START_CONFIG_DEFAULT();
    // cfg.esp_touch_v2_enable_crypt = true;
    // cfg.esp_touch_v2_key = "1234567890123456"; // Atur kunci enkripsi 16 byte

    // Jalankan layanan konfigurasi pintar
                                                  ...
                                                  int32_t event_id, void *event_data)
{
    ...
    if (event_base == SC_EVENT){
        ...
        case SC_EVENT_GOT_SSID_PSWD:{
            ...
            // Penyambungan Wi-Fi akan gagal pada tahap ini, jadi tidak dijalankan
            self->Save(ssid, password);
            // Tunda keluar dari mode konfigurasi
            xTaskCreate([](void *ctx){
...
#endif // Bukan untuk target ESP32P4

void WifiConfigurationAp::Stop() {
#if !CONFIG_IDF_TARGET_ESP32P4
    // Hentikan layanan konfigurasi pintar
    ...
    // Hentikan Wi-Fi tanpa deinit, karena penggeraknya dikelola WifiManager
    esp_wifi_stop();
    
    // Hancurkan antarmuka jaringan
    if (ap_netif_) {
