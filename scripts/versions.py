...
# Pindah ke akar proyek agar semua path relatif konsisten.
...
def get_app_desc(data):
    ...
    if magic != 0xabcd5432:
        raise Exception("Magic deskripsi aplikasi tidak valid")
...
def read_binary(dir_path):
    ...
    if app_partition is None:
        print("tidak ada partisi aplikasi yang ditemukan")
        return
    ...
    assert offset < len(app_data), "offset melampaui batas data"

    # Tambahkan ukuran checksum.
    image_size += 1
    image_size = (image_size + 15) & ~15
    # Tambahkan ukuran hash jika disertakan.
    if app_data[0x17] == 1:
        image_size += 32
    print(f"ukuran image: {image_size}")

    # Pastikan sisa ruang hanya berisi 0xFF.
    for i in range(image_size, len(app_data)):
        if app_data[i] != 0xFF:
            print(f"verifikasi image gagal, data di 0x{i:08x} bukan 0xFF")
            return

    image_data = app_data[:image_size]

    # Ekstrak image aplikasi tunggal jika belum ada.
    bin_path = os.path.join(dir_path, "xiaozhi.bin")
    if not os.path.exists(bin_path):
        print("mengekstrak berkas bin ke", bin_path)
        open(bin_path, "wb").write(image_data)

    # Deskripsi aplikasi tersimpan pada segmen pertama.
...
def upload_dir_to_oss(source_dir, target_dir):
    ...
    for filename in os.listdir(source_dir):
        ...
        if not os.path.isfile(os.path.join(source_dir, filename)):
            continue
        print('mengunggah', oss_key)
        bucket.put_object(oss_key, open(os.path.join(source_dir, filename), 'rb'))


def post_info_to_server(info):
    """
    Kirim informasi firmware ke server versi.

    Argumen:
        info: kamus yang memuat metadata firmware.
    """
    try:
        # Ambil alamat server dan token dari variabel lingkungan.
        server_url = os.environ.get('VERSIONS_SERVER_URL')
        server_token = os.environ.get('VERSIONS_TOKEN')

        if not server_url or not server_token:
            raise Exception("SERVER_URL atau TOKEN belum tersedia di variabel lingkungan")

        # Siapkan header dan muatan permintaan.
        headers = {
        ...
        # Pastikan respons server sukses.
        response.raise_for_status()

        print(f"berhasil mengunggah informasi versi untuk tag: {info['tag']}")
        ...
        if hasattr(e.response, 'json'):
            ...
            error_msg = str(e)
        print(f"gagal mengunggah informasi versi: {error_msg}")
        raise
    except Exception as e:
        print(f"terjadi kesalahan saat mengunggah informasi versi: {str(e)}")
        raise


def main():
    release_dir = "releases"
    # Cari semua arsip ZIP rilis yang diawali huruf v.
