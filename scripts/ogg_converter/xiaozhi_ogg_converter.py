...
class AudioConverterApp:
    def __init__(self, master):
        self.master = master
        master.title("Alat Konversi Batch Audio OGG")
    ...
    def select_files(self):
        file_types = [
            ("Berkas Audio", "*.wav *.mogg *.ogg *.flac") if self.mode.get() == "audio_to_ogg" else ("Berkas OGG", "*.ogg")
        ]
    ...
    def start_conversion(self, convert_all):
        ...
        try:
            if self.mode.get() == "audio_to_ogg":
                thread = threading.Thread(target=self.convert_audio_to_ogg, args=(input_files,))
            else:
    ...
    def convert_audio_to_ogg(self, input_files):
        """Logika konversi dari audio ke OGG."""
    ...
    def convert_ogg_to_audio(self, input_files):
        """Logika konversi dari OGG ke audio."""
        for input_path in input_files:
            try:
                filename = os.path.basename(input_path)
                base_name = os.path.splitext(filename)[0]
